...
const Tool_view = () => {
  ...
  useEffect(() => {
    ...
    const fetchTool = async () => {
      try {
        const res = await axios.get(`http://localhost:8070/tools/${id}`);
        setTool(res.data);
      } catch (error) {
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="col-span-2 row-span-1">
              {tool.tool_photos?.[1] && (
                <img
                  src={`http://localhost:8070/uploads/${tool.tool_photos[1]}`}
                  alt={tool.tool_title}
                  className="object-cover w-full h-full"
                />
              )}
            ...
            <div className="col-span-2 row-span-1">
              {tool.tool_photos?.[2] && (
                <img
                  src={`http://localhost:8070/uploads/${tool.tool_photos[2]}`}
                  alt={tool.tool_title}
                  className="object-cover w-full h-full"
                />
              )}
